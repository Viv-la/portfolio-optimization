...
# SARIMA model
sarima_model, sarima_forecast, sarima_metrics = train_sarima_model(tsla)
sarima_forecast.to_csv("data/processed/tsla_sarima_test_forecast.csv")
sarima_metrics.to_csv("data/processed/sarima_results.csv", index=False)

# Future forecast using best statistical baseline
future_forecast, confidence_intervals = forecast_future_prices(arima_model, steps=180)
future_forecast.to_csv("data/processed/tsla_future_forecast.csv")
confidence_intervals.to_csv("data/processed/tsla_forecast_confidence_intervals.csv")
...
# Model comparison
model_comparison = pd.concat(
    [arima_metrics, sarima_metrics, lstm_metrics],
    ignore_index=True
)
